/****** Object:  StoredProcedure [dbo].[sp_findMissingIndex]    Script Date: 1/23/2023 13:54:19 ******/
SET ANSI_NULLS ON
GO
...
-- Description: Finner indekser som mangler. Gå over statement før kjøring av et resultat. 
-- Den er streng slik at den vil ha for mange indekser, så ikke ta alt for god fisk...
-- Før kjøring av proc kan denne kjøres manuelt: sp_updatestats go
-- Changelog:  17.11.2021, <author>, <change>
-- ===============================================================================================================
--exec [dbo].[sp_findMissingIndex]
ALTER PROCEDURE [dbo].[sp_findMissingIndex]
AS
BEGIN
	...
	SELECT 
	  ...
	  '  CREATE INDEX
		   [IX_' + COALESCE(SPs.referenced_schema_name,OBJECT_SCHEMA_NAME(mid.object_id),'')+
		COALESCE(SPs.referenced_entity_name,OBJECT_NAME(mid.object_id),'') + LEFT(REPLACE(ISNULL ('_' + mid.equality_columns,'1'),'[',''),6) + ']
		 ON ' + mid.statement + '
	...
	LEFT JOIN (
	  ...
	  --WHERE SPs.total_elapsed_time*1.0/290/1000/SPs.exec_count/425 > 1
	  ORDER BY SPs.total_elapsed_time*1.0/290/1000/SPs.exec_count/425 DESC
